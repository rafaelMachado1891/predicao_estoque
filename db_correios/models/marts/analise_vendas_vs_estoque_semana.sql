...
SELECT
    codigo,
    referencia,
    COUNT(semana_ano) AS contagem_semana,
    SUM(total_vendido) AS quantidade_vendida,
    SUM(numero_pedidos) AS numero_de_pedidos,    
    estoque_minimo

FROM  agrupamento_filtro
WHERE obs_estoque = 'vendas_excederam_estoque'
GROUP BY
    codigo,
    referencia,
    estoque_minimo
